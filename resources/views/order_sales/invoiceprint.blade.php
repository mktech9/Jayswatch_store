    ...
    <table style="width: 100%;padding:20px 20px 5px 20px;text-align:left;">
        <tr>
            <td>
                ...
                <span class="style"
                    style="transform: ScaleX(1.05);">{{ $order->order_id }}</span>&nbsp;&nbsp;&nbsp;&nbsp; <br>
            ...
            @php
                $logoPath = public_path('assets/front/img/logo.png');
                $logoBase64 = null;

                if (file_exists($logoPath)) {
                    $type = pathinfo($logoPath, PATHINFO_EXTENSION);
                    $data_img = file_get_contents($logoPath);
                    $logoBase64 = 'data:image/' . $type . ';base64,' . base64_encode($data_img);
                }
            @endphp

            <td style="padding-top:1rem;padding-bottom:1rem;text-align:right;">
                @if ($logoBase64)
                    <img src="{{ $logoBase64 }}" style="width:8rem;">
                @endif
            </td>
        </tr>
        <tr>
